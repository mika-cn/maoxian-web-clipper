(function (root, factory) {
  ...
  const values = {
    ...
    "label.html-capture-object-input": "OBJECT elements (are used to include some complicated files which are so difficult to render taht the browser needs to install plugins to corrently handle it. and will rarely include images, audios or videos. We suggest you only save images)",
    "label.html-capture-applet-input": "APPLET elements (are used to include JAVA applets which are not longer supported by most modern browsers. We suggest you don't save them)",
