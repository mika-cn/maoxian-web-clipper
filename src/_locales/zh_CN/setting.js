(function (root, factory) {
  ...
  const values = {
    ...
    "notice.capture-filter-list-format": "「过滤器组」由多个过滤器组成，裁剪时会依次进行匹配，当其中一个过滤器成功匹配，后面的过滤器会直接被忽略。<br />多个过滤器中间用竖杠符号（|）隔开（如：<code>过滤器A|过滤器B|过滤器C</code>。）",
    ...
    "notice.web-font.intro": "Web 字体是网页嵌入的字体文件，常用于改变网页文本的字体样式，也会用于引入图标。",
