...
const Dict   = getDict(MimeDB);
const MyDict = getDict(MyMimeDB);

/**
 * @param {String} type (not include parameters)
 */
function getExtension(type) {
  let it = MyMimeDB[type];
  if (it && it.extensions) {
    return it.extensions[0];
  } else {
    it = MimeDB[type]
    return (it && it.extensions ? it.extensions[0] : null);
  }
}

function getType(extension) {
  let it = MyDict[extension];
  return it ? it : Dict[extension];
}

export default {getExtension, getType}
