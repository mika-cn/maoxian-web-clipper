...
for fname in package.json yarn.lock webpack.config.js README.md README-DEV.md LICENSE; do
  cp $dir/$fname $dist/$extname/$fname
done
cp -r $dir/src $dist/$extname
rm -f "$dist/$extname/manifest-chromium.json"
...
echo "1. install dependencies (you need to have node, npm and yarn installed)" >> $reviewer_file_path
echo "" >> $reviewer_file_path
echo "\`\`\`shell" >> $reviewer_file_path
echo "yarn install" >> $reviewer_file_path
echo "\`\`\`" >> $reviewer_file_path
echo "" >> $reviewer_file_path
...
echo "" >> $reviewer_file_path
echo "\`\`\`shell" >> $reviewer_file_path
echo "MX_FIREFOX_ID=$MX_FIREFOX_ID npm run build-firefox-production" >> $reviewer_file_path
echo "\`\`\`" >> $reviewer_file_path
echo "" >> $reviewer_file_path
